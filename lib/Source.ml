...
type t =
  { text : string
  ; tokens : (Parser.token * Location.t) list
  }
...
let create text =
  ...
  let rec loop acc =
    ...
    | tok ->
        loop ((tok, Location.of_lexbuf lexbuf) :: acc)
  in
  { text
  ; tokens = loop [] }
...
let position_before (t : t) (pos : Lexing.position) =
  ...
  let maybe_check_alphanum c =
    if Char.is_alphanum t.text.[pos.pos_cnum] then false else Char.is_alphanum c
  in
  let f _ c =
...
let tokens_between (t : t) ~filter loc_start loc_end =
  let l = t.tokens in
  let l = List.drop_while l ~f:(fun (_,tok_loc) ->
    Position.compare tok_loc.loc_start loc_start < 0)
  in
  let l = List.take_while l ~f:(fun (_,tok_loc) ->
    Position.compare tok_loc.loc_end loc_end <= 0)
  in
  List.filter_map ~f:(fun ((tok,_) as x) -> if filter tok then Some x else None) l
